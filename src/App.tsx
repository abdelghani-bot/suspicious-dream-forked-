...
function CustomersModule({ customers, setCustomers, showToast, sales = [] }) {
  ...
  const blank = {
    id: "", name: "", phone: "", taxId: "",
    totalSpent: 0, visits: 0, lastVisit: "-",
    category: "individual", children_count: "", children_ages: [],
  };
  const [form, setForm] = useState(blank);
  const F = (k, v) => setForm(p => ({ ...p, [k]: v }));
  ...
  const computeStats = (customerId) => {
    ...
    const monthlyVisits = cSales.filter(s => s.created_at?.startsWith(thisMonthKey)).length;
    ...
    // RFM — آخر 3 شهور
    const recent = cSales.filter(s => new Date(s.created_at) >= threeMonthsAgo);
    const freq3 = recent.length;
    const monetary3 = recent.reduce((s, sale) => s + (sale.subtotal || 0), 0);

    const rScore = daysSinceLast <= 14 ? 40 : daysSinceLast <= 30 ? 30 : daysSinceLast <= 90 ? 15 : 0;
    const fScore = freq3 > 10 ? 30 : freq3 >= 5 ? 20 : freq3 >= 2 ? 10 : 0;
    const mScore = monetary3 > 1000 ? 30 : monetary3 >= 500 ? 20 : monetary3 >= 200 ? 10 : 0;
    const rfmScore = rScore + fScore + mScore;

    const vipLevel = rfmScore >= 80 ? "vip"
      : rfmScore >= 55 ? "excellent"
      : rfmScore >= 30 ? "good" : "weak";

    const status = totalVisits === 1 && daysSinceLast <= 30 ? "new"
      : daysSinceLast <= 30 ? "regular"
      : daysSinceLast <= 90 ? "at_risk"
      : "inactive";

    const lastItems = lastSale?.items
      ? (typeof lastSale.items === "string" ? JSON.parse(lastSale.items) : lastSale.items)
      : [];

    return {
      totalVisits, monthlyVisits, totalSpent, monthlySpent, avgInvoice,
      lastVisitDate, daysSinceLast, rfmScore, vipLevel, status, lastItems,
    };
  };
  ...
  const todayIds = [...new Set(sales.filter(s => s.created_at?.startsWith(todayKey)).map(s => s.customer).filter(Boolean))];
  const todayCustomers = enriched.filter(c => todayIds.includes(c.id));

  // ===== المختفون =====
  const inactiveCustomers = enriched.filter(c => c.stats?.status === "inactive");
  ...
  const BarChart = ({ title, data }) => {
    ...
    return (
      <div style={{ background: "#0f1623", border: "1px solid #1d2d4a", borderRadius: 14, padding: 16 }}>
        <div style={{ fontWeight: 700, color: "#dde8ff", fontSize: 14, marginBottom: 14 }}>{title}</div>
        {data.map(d => (
          <div key={d.label} style={{ marginBottom: 12 }}>
            ...
            <div style={{ background: "#080e1a", borderRadius: 4, height: 8 }}>
              <div style={{ background: d.color, height: "100%", borderRadius: 4, width: `${(d.count / max) * 100}%`, transition: "width 0.5s" }} />
            </div>
          </div>
        ))}
      </div>
  ...
  const CustomerCard = ({ c }) => {
    ...
    return (
      <div style={{ background: "#0f1623", border: `1px solid ${vip ? vip.color + "33" : "#1d2d4a"}`, borderRadius: 14, padding: 18 }}>

        {/* رأس الكارت */}
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{ width: 42, height: 42, borderRadius: 10, background: "#1a2a5a", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20 }}>
              {c.category === "individual" ? "👤" : c.category === "family_no_kids" ? "👫" : "👨‍👩‍👧"}
            </div>
            <div>
              <div style={{ fontWeight: 700, color: "#dde8ff", fontSize: 14 }}>{c.name}</div>
              <div style={{ color: "#3a6a9a", fontSize: 11 }}>{c.id} • {c.phone}</div>
            </div>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 4, alignItems: "flex-end" }}>
            {vip && <span style={{ background: vip.bg, color: vip.color, padding: "3px 8px", borderRadius: 6, fontSize: 11, fontWeight: 700 }}>{vip.label}</span>}
            {statusC && <span style={{ background: "#080e1a", color: statusC.color, padding: "3px 8px", borderRadius: 6, fontSize: 11 }}>{statusC.label}</span>}
          </div>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 6, marginBottom: 10 }}>
          {[
            { label: "إجمالي الزيارات", value: s?.totalVisits || 0, color: "#5a9aff" },
            { label: "زيارات الشهر",    value: s?.monthlyVisits || 0, color: "#44dd88" },
            { label: "متوسط الفاتورة",  value: s ? s.avgInvoice.toFixed(0) + " ر.س" : "-", color: "#a78bfa" },
            { label: "إجمالي المشتريات", value: s ? s.totalSpent.toFixed(0) + " ر.س" : "-", color: "#ffd700" },
            { label: "مشتريات الشهر",   value: s ? s.monthlySpent.toFixed(0) + " ر.س" : "-", color: "#ffaa44" },
            { label: "آخر زيارة",       value: s ? `${s.daysSinceLast} يوم` : "لم يزر", color: "#7a9aaa" },
          ].map(item => (
            <div key={item.label} style={{ background: "#080e1a", borderRadius: 8, padding: "7px 8px" }}>
              <div style={{ color: "#3a5a8a", fontSize: 9 }}>{item.label}</div>
              <div style={{ color: item.color, fontWeight: 700, fontSize: 13, marginTop: 2 }}>{item.value}</div>
            </div>
          ))}
        </div>
        ...
        {s && (
          <div style={{ marginBottom: 10 }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 3 }}>
              ...
              <span style={{ color: vip?.color, fontSize: 11, fontWeight: 700 }}>{s.rfmScore}/100</span>
            </div>
            <div style={{ background: "#080e1a", borderRadius: 4, height: 5 }}>
              <div style={{ background: vip?.color || "#4a6a8a", height: "100%", borderRadius: 4, width: `${s.rfmScore}%`, transition: "width 0.5s" }} />
            </div>
          </div>
        )}
        ...
        {s?.lastItems?.length > 0 && (
          <div style={{ marginBottom: 10 }}>
            <button
              ...
              style={{ background: "transparent", border: "none", color: "#4a6a8a", fontSize: 11, cursor: "pointer", padding: 0 }}
            >
              {isExpanded ? "▲ إخفاء" : `▼ آخر مشتريات (${s.lastItems.length} صنف)`}
            </button>
            {isExpanded && (
              <div style={{ marginTop: 6, display: "flex", flexWrap: "wrap", gap: 4 }}>
                {s.lastItems.map((item, i) => (
                  <span key={i} style={{ background: "#080e1a", color: "#5a9adf", padding: "3px 8px", borderRadius: 6, fontSize: 10 }}>
                    {item.name} × {item.qty}
                  </span>
                ))}
              </div>
        ...
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
          <button
            ...
            style={{ background: "#0a2a0a", border: "1px solid #1a4a1a", borderRadius: 8, padding: "6px 12px", color: "#44dd88", fontSize: 12, cursor: "pointer", fontWeight: 700 }}
          >
            📱 واتساب
          </button>
          <button
            onClick={() => openEdit(c)}
            style={{ background: "#0a1a3a", border: "1px solid #1d2d4a", borderRadius: 8, padding: "6px 12px", color: "#5a9aff", fontSize: 12, cursor: "pointer" }}
          >
            ✏️ تعديل
          </button>
          <button
            ...
            style={{ background: "#2a0a0a", border: "1px solid #3a1010", borderRadius: 8, padding: "6px 12px", color: "#ff4444", fontSize: 12, cursor: "pointer" }}
          >
            🗑️ حذف
          </button>
        </div>
  ...
  const openAdd = () => {
    setEditing(null);
    setForm({ ...blank, id: "C" + String(customers.length + 1).padStart(3, "0") });
    setShowForm(true);
  };
  const openEdit = (c) => { setEditing(c.id); setForm({ ...blank, ...c }); setShowForm(true); };

  const save = async () => {
    if (!form.name || !form.phone) { showToast("يرجى ملء بيانات العميل", "error"); return; }
    const saved = {
      ...form,
      totalSpent: form.totalSpent || 0, visits: form.visits || 0, lastVisit: form.lastVisit || "-",
      children_count: form.category === "family_with_kids" ? form.children_count : null,
      children_ages: form.category === "family_with_kids" ? form.children_ages : [],
    };
    if (editing) {
      const { error } = await supabase.from("customers").update(saved).eq("id", editing);
      if (error) { showToast("خطأ في التعديل: " + error.message, "error"); return; }
      setCustomers(p => p.map(x => (x.id === editing ? saved : x)));
    } else {
      const { data, error } = await supabase.from("customers").insert(saved).select();
  ...
  const toggleAge = (age) => {
    ...
    F("children_ages", current.includes(age) ? current.filter(a => a !== age) : [...current, age]);
  };
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18 }}>
        <h2 style={{ margin: 0, fontSize: 20, fontWeight: 800 }}>إدارة العملاء</h2>
        <Btn icon="plus" onClick={openAdd}>إضافة عميل</Btn>
      </div>

      {/* بطاقات الإحصائيات */}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 10, marginBottom: 20 }}>
        {[
          { label: "إجمالي العملاء", value: totalCustomers, color: "#5a9aff",  icon: "👥" },
          { label: "جديد هذا الشهر",  value: newCount,       color: "#44dd88", icon: "🆕" },
          { label: "عملاء VIP",        value: vipCount,       color: "#ffd700", icon: "👑" },
          { label: "مختفون",           value: inactiveCount,  color: "#ff4444", icon: "💤" },
        ].map(item => (
          <div key={item.label} style={{ background: "#0f1623", border: "1px solid #1d2d4a", borderRadius: 12, padding: "14px 16px" }}>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 16, flexWrap: "wrap" }}>
        <button style={tabBtn("cards")}    onClick={() => setActiveTab("cards")}>📋 كل العملاء</button>
        <button style={tabBtn("today")}    onClick={() => setActiveTab("today")}>📅 عملاء اليوم {todayCustomers.length > 0 && `(${todayCustomers.length})`}</button>
        <button style={tabBtn("inactive")} onClick={() => setActiveTab("inactive")}>💤 المختفون {inactiveCount > 0 && `(${inactiveCount})`}</button>
        <button style={tabBtn("charts")}   onClick={() => setActiveTab("charts")}>📊 الرسوم البيانية</button>
      </div>
        ...
        <>
          <div style={{ display: "flex", gap: 10, marginBottom: 14, flexWrap: "wrap" }}>
            <input
              ...
              style={{ flex: 1, minWidth: 200, background: "#080e1a", border: "1px solid #1d2d4a", borderRadius: 8, padding: "9px 14px", color: "#dde8ff", fontSize: 14, outline: "none" }}
            />
            <select value={filterVip} onChange={e => setFilterVip(e.target.value)}
              style={{ background: "#080e1a", border: "1px solid #1d2d4a", borderRadius: 8, padding: "9px 12px", color: "#dde8ff", fontSize: 13 }}>
      ...
      {activeTab === "today" && (
        <div>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
            <h3 style={{ margin: 0, color: "#dde8ff", fontSize: 15 }}>عملاء اليوم ({todayCustomers.length})</h3>
            {todayCustomers.length > 0 && (
              <button
                onClick={() => sendBulk(todayCustomers, "مرحباً! شكراً لزيارتكم اليوم 😊 نتمنى أن تكونوا بخير")}
                style={{ background: "#0a2a0a", border: "1px solid #1a4a1a", borderRadius: 8, padding: "8px 16px", color: "#44dd88", fontSize: 13, cursor: "pointer", fontWeight: 700 }}
              >
                📣 تواصل جماعي
              </button>
            )}
          ...
          {todayCustomers.length === 0
            ? <div style={{ color: "#3a5a8a", textAlign: "center", padding: 40 }}>لا يوجد عملاء اليوم</div>
            : todayCustomers.map(c => {
                const vip = c.stats ? vipConfig[c.stats.vipLevel] : null;
                return (
                  <div key={c.id} style={{ background: "#0f1623", border: "1px solid #1d2d4a", borderRadius: 12, padding: "12px 16px", display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      <span style={{ fontSize: 20 }}>{c.category === "individual" ? "👤" : "👨‍👩‍👧"}</span>
                      <div>
                        <div style={{ fontWeight: 700, color: "#dde8ff" }}>{c.name}</div>
                        <div style={{ color: "#4a6a8a", fontSize: 11 }}>{c.phone}</div>
                      </div>
                      {vip && <span style={{ background: vip.bg, color: vip.color, padding: "2px 8px", borderRadius: 6, fontSize: 11 }}>{vip.label}</span>}
                    </div>
                    <button
                      onClick={() => openWhatsApp(c.phone, `مرحباً ${c.name}! شكراً لزيارتكم اليوم 😊`)}
                      style={{ background: "#0a2a0a", border: "1px solid #1a4a1a", borderRadius: 8, padding: "6px 14px", color: "#44dd88", fontSize: 12, cursor: "pointer" }}
                    >
                      📱 واتساب
                    </button>
                  </div>
      ...
      {activeTab === "inactive" && (
        <div>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
            <h3 style={{ margin: 0, color: "#dde8ff", fontSize: 15 }}>العملاء المختفون ({inactiveCustomers.length})</h3>
            {inactiveCustomers.length > 0 && (
              <button
                onClick={() => sendBulk(inactiveCustomers, "مرحباً! نفتقدكم في صيدليتنا 💊 لدينا عروض خاصة تنتظركم 🎁")}
                style={{ background: "#2a0a0a", border: "1px solid #3a1010", borderRadius: 8, padding: "8px 16px", color: "#ff6644", fontSize: 13, cursor: "pointer", fontWeight: 700 }}
              >
                📣 حملة استرداد جماعي
              </button>
            )}
          ...
          {inactiveCustomers.map(c => {
            const s = c.stats;
            return (
              <div key={c.id} style={{ background: "#0f1623", border: "1px solid #2a1010", borderRadius: 12, padding: "12px 16px", display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                <div>
                  <div style={{ fontWeight: 700, color: "#dde8ff" }}>{c.name}</div>
                  <div style={{ color: "#4a6a8a", fontSize: 11, marginTop: 3 }}>
                    غائب منذ <span style={{ color: "#ff4444" }}>{s?.daysSinceLast} يوم</span> •
                    إجمالي مشتريات: <span style={{ color: "#ffd700" }}>{s?.totalSpent.toFixed(0)} ر.س</span>
                  </div>
                  <div style={{ display: "flex", gap: 4, marginTop: 5, flexWrap: "wrap" }}>
                    {s?.lastItems?.slice(0, 3).map((item, i) => (
                      <span key={i} style={{ background: "#080e1a", color: "#5a7a9a", padding: "2px 6px", borderRadius: 4, fontSize: 10 }}>
                        {item.name}
                      </span>
                    ))}
                  </div>
                ...
                <button
                  onClick={() => openWhatsApp(c.phone, `مرحباً ${c.name}! نفتقدكم 💊 لدينا عروض خاصة تنتظركم`)}
                  style={{ background: "#0a2a0a", border: "1px solid #1a4a1a", borderRadius: 8, padding: "6px 14px", color: "#44dd88", fontSize: 12, cursor: "pointer" }}
                >
                  📱 استرداد
                </button>
      ...
      {activeTab === "charts" && (
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
          <BarChart title="📊 توزيع نوع العملاء" data={[
            { label: "👤 فرد",              count: customers.filter(c => c.category === "individual").length,      color: "#a78bfa" },
            { label: "👫 أسرة بدون أطفال", count: customers.filter(c => c.category === "family_no_kids").length,  color: "#5a9aff" },
            { label: "👨‍👩‍👧 أسرة مع أطفال",  count: customers.filter(c => c.category === "family_with_kids").length, color: "#44dd88" },
          ]} />
          <BarChart title="📊 حالة العملاء" data={[
            { label: "🆕 جديد",    count: enriched.filter(c => c.stats?.status === "new").length,      color: "#44dd88" },
            { label: "✅ منتظم",  count: enriched.filter(c => c.stats?.status === "regular").length,  color: "#5a9aff" },
            { label: "⚠️ في خطر", count: enriched.filter(c => c.stats?.status === "at_risk").length,  color: "#ffaa44" },
            { label: "💤 مختفي",  count: enriched.filter(c => c.stats?.status === "inactive").length, color: "#ff4444" },
          ]} />
          <BarChart title="👑 تصنيف VIP" data={[
            { label: "👑 VIP",   count: enriched.filter(c => c.stats?.vipLevel === "vip").length,       color: "#ffd700" },
            { label: "⭐ ممتاز", count: enriched.filter(c => c.stats?.vipLevel === "excellent").length, color: "#5a9aff" },
            { label: "✅ جيد",   count: enriched.filter(c => c.stats?.vipLevel === "good").length,      color: "#44dd88" },
            { label: "🔴 ضعيف", count: enriched.filter(c => c.stats?.vipLevel === "weak").length,      color: "#ff4444" },
          ]} />
        </div>
      )}

      {/* مودال الإضافة/التعديل */}
      <Modal open={showForm} onClose={() => setShowForm(false)} title={editing ? "تعديل العميل" : "إضافة عميل جديد"}>
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          <Input label="اسم العميل *"            value={form.name}   onChange={v => F("name", v)}   placeholder="الاسم الكامل" />
          <Input label="رقم الهاتف *"            value={form.phone}  onChange={v => F("phone", v)}  placeholder="05XXXXXXXX" />
          <Input label="الرقم الضريبي (اختياري)" value={form.taxId}  onChange={v => F("taxId", v)}  placeholder="اختياري" />
          <div>
            <div style={{ color: "#7a9aaa", fontSize: 12, marginBottom: 8 }}>نوع العميل *</div>
            <div style={{ display: "flex", gap: 8 }}>
              {[
                ...
                <div key={opt.val} onClick={() => F("category", opt.val)}
                  style={{ flex: 1, padding: "10px 8px", borderRadius: 10, border: `2px solid ${form.category === opt.val ? "#3a9aff" : "#1d2d4a"}`, background: form.category === opt.val ? "#0a1a3a" : "#080e1a", color: form.category === opt.val ? "#3a9aff" : "#5a7a9a", fontSize: 12, textAlign: "center", cursor: "pointer" }}>
                  {opt.label}
                </div>
              ))}
            </div>
          ...
          {form.category === "family_with_kids" && (
            <>
              <Input label="عدد الأطفال *" value={form.children_count} onChange={v => F("children_count", v)} placeholder="مثال: 2" type="number" />
              <div>
                <div style={{ color: "#7a9aaa", fontSize: 12, marginBottom: 8 }}>الفئات العمرية</div>
                <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                  {["أقل من سنة", "1-3 سنوات", "4-6 سنوات", "7-12 سنة", "13-17 سنة"].map(age => {
                    const selected = (form.children_ages || []).includes(age);
                    return (
                      <div key={age} onClick={() => toggleAge(age)}
                        style={{ padding: "7px 12px", borderRadius: 20, border: `1px solid ${selected ? "#44dd88" : "#1d2d4a"}`, background: selected ? "#0a2a1a" : "#080e1a", color: selected ? "#44dd88" : "#5a7a9a", fontSize: 12, cursor: "pointer" }}>
                        {selected ? "✓ " : ""}{age}
                      </div>
                    );
                  })}
        ...
        <div style={{ display: "flex", gap: 10, marginTop: 18, justifyContent: "flex-end" }}>
          ...
          <Btn icon="check" onClick={save}>{editing ? "حفظ التعديل" : "إضافة العميل"}</Btn>
        </div>
      </Modal>
    </div>
  );
