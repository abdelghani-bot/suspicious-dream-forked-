...
export function InventoryStatement({
    ...
    // ===== 🆕 قايمة الأصناف اللي محتاجة تسوية (variance) — جايه من الكاش/الأوفلاين =====
    const [pendingVariance, setPendingVariance] = useState([]);

    useEffect(() => {
        if (!pharmacyId) return;
        getPendingZeroStockVariance(pharmacyId).then(({ data, fromCache }) => {
            setPendingVariance(data);
            if (fromCache) showToast("📴 قايمة الأصناف المحتاجة تسوية محمّلة من الكاش المحلي", "warning");
        });
    }, [pharmacyId]);
    ...
    const baseRows = (products ?? []).flatMap((p) => {
    ...
    // ===== 🆕 أصناف مسجلة كـ "محتاجة تسوية" (كميتها اتصفرت أو variance معلّق) لكنها مش ظاهرة أصلاً في baseRows =====
    const zeroStockRows = pendingVariance
        .filter((v) => !baseRows.some((r) => r.productId === v.product_id))
        .map((v) => {
            const p = (products ?? []).find((x) => x.id === v.product_id);
            if (!p) return null;
            return {
                key: `${p.id}::needsAdjustment`,
                productId: p.id,
                batchIndex: null,
                name: p.name,
                barcode: p.barcode ?? "-",
                ...
                price: p.price ?? 0,
                batchNumber: null,
                needsAdjustment: true,   // 🆕 استخدمه في الجدول للتلوين لما نراجعه
                varianceNote: v.notes,
            };
        })
        .filter(Boolean);

    const allRows = [...baseRows, ...zeroStockRows];
    ...
    const saveAdjustment = async () => {
        ...
        try {
            ...
            const logData = {
                ...
                type: "تسوية سريعة",
                items: [{
                    id: prod.id, name: prod.name, expiry: adjExpiry || null,
                    systemQty: editingRow.stock, actualQty: newQty, diff,
                }],
                notes: adjNote || null,
                by: currentUser?.name,
                pharmacy_id: pharmacyId,
            };
            const adjustments = diff !== 0 ? [{
                inventory_log_id: logData.id, product_id: prod.id, quantity: diff,
                date: logData.date, created_by: currentUser?.name, pharmacy_id: pharmacyId,
            }] : [];

            const result = await queueEvent({
                id: crypto.randomUUID(),
                type: "INVENTORY_COUNT_SAVE",
                pharmacy_id: pharmacyId, // top-level — إلزامي لأي event جديد
                timestamp: new Date().toISOString(),
                payload: {
                    logData,
                    adjustments,
                    productUpdates: [{ id: prod.id, pharmacy_id: pharmacyId, stock: newStock, batches: updatedBatches }],
                    resolveVariance: [{
                        productId: prod.id,
                        resolvedBy: currentUser?.name || null,
                        resolutionNotes: `اتقفلت تلقائيًا بعد تسوية سريعة من كشف المخزون (${editingRow.stock} → ${newQty})`,
                    }],
                },
            });
            if (!result.synced) {
                showToast("📴 التسوية اتحفظت محليًا - هتتزامن لما النت يرجع", "warning");
            }

            try {
                await window.offlineAPI?.removeVarianceLogCacheByProduct?.({ pharmacyId, productId: prod.id });
            } catch (err) {
                console.error("removeVarianceLogCacheByProduct failed:", err);
            }
            setPendingVariance((prev) => prev.filter((v) => v.product_id !== prod.id));

            setProducts((prev) =>
                prev.map((p) => (p.id === prod.id ? { ...p, stock: newStock, batches: updatedBatches } : p))
            );

            logAudit({
                pharmacyId, userName: currentUser?.name, action: "update",
                entityType: "product", entityId: prod.id, entityLabel: prod.name,
                ...
                description:
                    ...
                    (editingRow.expiry !== (adjExpiry || null) ? `، الصلاحية من ${editingRow.expiry || "-"} إلى ${adjExpiry || "-"}` : "") +
        ...
        try {
            ...
            const logData = {
                ...
                items: [{
                    ...
                    systemQty: settlingProduct.totalQty, actualQty: newTotal, diff, batchChanges,
                }],
            ...
            const result = await queueEvent({
                ...
                payload: {
                    ...
                    resolveVariance: [{
                        productId: prod.id,
                        resolvedBy: currentUser?.name || null,
                        resolutionNotes: `اتقفلت تلقائيًا بعد تسوية سريعة (صنف كامل) من كشف المخزون (${settlingProduct.totalQty} → ${newTotal})`,
                    }],
                },
            });
            if (!result.synced) {
