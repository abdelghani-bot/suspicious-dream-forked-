...
export function POS({
    ...
    // 🆕 بانر مساعد الصيدلي: إحصائيات العميل (نمط الشراء/متوسط الفاتورة/اتجاه الشراء)
    // بيستخدم نفس دالة computeCustomerStats الموجودة في قسم العملاء — من غير تكرار للمنطق.
    // creditPayments مش لازمة هنا لأن الحقول المعروضة في POS ما بتحتاجش حساب المديونية.
    const customerStats = useMemo(
        () => (inv.selCustomer ? computeCustomerStats(inv.selCustomer, sales) : null),
        [inv.selCustomer, sales]
    );
    ...
    const completeSale = async () => {
        ...
        const invoice = {
            ...
            // 🆕 صورة الوصفة (مضغوطة/مصغّرة) — بتتحفظ مع الفاتورة عشان ترجع تتفتح تاني وقت مراجعة الفاتورة
            prescription_img: inv.prescriptionImg || null,
            patient_name: inv.patientName || null,
        };
        ...
        // 🆕 كاش محلي قابل للقراءة الفورية (sales_cache) — منفصل عن طابور المزامنة،
        // عشان تقدر تفتح/تدور على الفاتورة (وتشوف الوصفة المرفقة) وانت أوفلاين من غير
        // ما تستنى المزامنة أو تعمل JSON.parse على كل صفوف pending_sync_events.
        if (window.offlineAPI?.insertSaleCache) {
            await window.offlineAPI.insertSaleCache(invoice);
        }
                ...
                >
                    ...
                    <input
                        ...
                        onChange={async (e) => {
                            ...
                            if (!file) return;
                            try {
                                const compressed = await compressPrescriptionImage(file);
                                setInv((p) => ({ ...p, prescriptionImg: compressed }));
                            } catch {
                                showToast("تعذر معالجة صورة الوصفة", "error");
                            }
                        }}
                    />
                </div>

                {/* 🆕 بانر مساعد الصيدلي — لمحة سريعة عن سلوك شراء العميل وقت البيع (بدون نقاط الولاء، ظاهرة تحت أصلاً) */}
                {inv.selCustomer && customerStats && (
                    <div
                        style={{
                            padding: "5px 16px",
                            borderBottom: `1px solid ${COLORS.border}`,
                    ...
                    >
                        <span style={{ color: COLORS.textDim }}>
                            نمط الشراء:{" "}
                            <b style={{ color: customerStats.buyerType === "شامل" ? COLORS.green : COLORS.blue }}>
                                {customerStats.buyerType === "شامل" ? "🌐 شامل" : (customerStats.buyerType || "-")}
                            </b>
                        </span>
                        <span style={{ color: COLORS.textDim }}>
                            متوسط الفاتورة: <b style={{ color: COLORS.purple }}>{customerStats.avgInvoice.toFixed(0)} ر.س</b>
                        </span>
                        <span style={{ color: COLORS.textDim }}>
                            آخر زيارة: <b style={{ color: COLORS.textPrimary }}>{customerStats.daysSinceLast} يوم</b>
                        </span>
                        {customerStats.activeMonthsCount >= 2 && (
                            <span
                                title={trendConfig[customerStats.trendDirection].label}
                                style={{
                                    background: trendConfig[customerStats.trendDirection].bg,
                                    color: trendConfig[customerStats.trendDirection].color,
                                    padding: "2px 8px",
                                    borderRadius: 5,
                                    fontWeight: 700,
                                    fontSize: 12,
                            ...
                            >
                                {customerStats.trendDirection === "up" ? "↑" : customerStats.trendDirection === "down" ? "↓" : "→"}
                            </span>
                        )}
                    </div>
                )}
