...
export function TargetModule({ users, sales, customers, products, currentUser, pharmacyId, showToast, returns = [], canAdd = true, canEdit = true, canDelete = true }) {
    ...
    // ── تجميع الأصناف المحفزة حسب الـ Tier (لعرض/طباعة/مشاركة القائمة) ──
    const groupedIncentiveProducts = () =>
        [...tiers].sort((a, b) => b.threshold - a.threshold)
            .map((t) => ({ tier: t, products: autoIncentiveProducts.filter((x) => x.tier.id === t.id) }))
            .filter((g) => g.products.length > 0);

    const buildIncentiveListText = () => {
        let text = `🎯 أصناف التحفيز — ${incMonthKey}\n\n`;
        groupedIncentiveProducts().forEach((g) => {
            text += `▸ هامش ≥ ${g.tier.threshold}% (عمولة ${g.tier.rate}%)\n`;
            g.products.forEach(({ product: p, manual }) => {
                text += `  - ${p.name_ar || p.name || p.nameAr}${manual ? " (يدوي)" : ""}\n`;
            });
            text += `\n`;
        });
        return text;
    ...
    // ── فتح نافذة طباعة منسّقة لقائمة الأصناف المحفزة ──
    const printIncentiveList = () => {
        const win = window.open("", "_blank");
        if (!win) { showToast("فعّل النوافذ المنبثقة عشان تقدر تطبع", "error"); return; }
        const groups = groupedIncentiveProducts();
        const rows = groups.map((g) => `
      <h3>هامش ≥ ${g.tier.threshold}% — عمولة ${g.tier.rate}%</h3>
      <ul>
        ${g.products.map(({ product: p, manual }) => `<li>${p.name_ar || p.name || p.nameAr}${manual ? " <small>(يدوي)</small>" : ""}</li>`).join("")}
      </ul>
    `).join("");
        win.document.write(`
      <html dir="rtl" lang="ar">
      <head><meta charset="utf-8" /><title>أصناف التحفيز — ${incMonthKey}</title>
      <style>
      ...
      <body>
        <h1>🎯 أصناف التحفيز — ${incMonthKey}</h1>
        <div style="color:#666;font-size:12px;margin-bottom:10px;">إجمالي الأصناف: ${autoIncentiveProducts.length}</div>
        ${rows || "<div>لا توجد أصناف مستوفية حالياً</div>"}
      </body>
      </html>
    `);
        win.document.close();
    ...
    // ── مشاركة القائمة (Web Share API) أو نسخها للحافظة كبديل ──
    const shareIncentiveList = async () => {
        const text = buildIncentiveListText();
        if (navigator.share) {
            try { await navigator.share({ title: `أصناف التحفيز — ${incMonthKey}`, text }); }
            catch (e) { /* المستخدم لغى المشاركة */ }
        } else if (navigator.clipboard) {
            await navigator.clipboard.writeText(text);
            showToast("تم نسخ القائمة للحافظة ✓");
        } else {
            showToast("المشاركة غير مدعومة على هذا المتصفح", "error");
        }
    };
        ...
        <div>
                            ...
                            <div key={u.id} style={{ background: COLORS.surface, backdropFilter: "blur(16px)", WebkitBackdropFilter: "blur(16px)", border: `1px solid ${COLORS.border}`, borderRadius: 14, overflow: "hidden", marginBottom: 12 }}>
                                <div style={{ padding: 18 }}>
                                    ...
                                    <div style={{ borderTop: `1px solid ${COLORS.border}`, paddingTop: 14 }}>
                                        ...
                                        {/* -- مجموعة 1: أرقام البيع -- */}
                                        <div style={{ color: COLORS.textDim, fontSize: 10, fontWeight: 700, marginBottom: 6 }}>💰 أرقام البيع</div>
                                        ...
                                        {/* -- مجموعة 2: أرقام العملاء -- */}
                                        <div style={{ color: COLORS.textDim, fontSize: 10, fontWeight: 700, marginBottom: 6 }}>👥 أرقام العملاء</div>
            ...
            {subTab === "incentive" && (
                <div>
                    ...
                    <div style={incentiveCardStyle(COLORS.surfaceAlt)}>
                        <div onClick={() => setTiersExpanded((v) => !v)} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: tiersExpanded ? 10 : 0, cursor: "pointer" }}>
                            <div style={{ color: COLORS.purple, fontWeight: 700 }}>📶 مستويات العمولة (Tiers) <span style={{ color: COLORS.textDim, fontSize: 11, fontWeight: 400 }}>({tiers.length})</span></div>
                            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                                ...
                                <span style={{ color: COLORS.textDim, fontSize: 13 }}>{tiersExpanded ? "▲" : "▼"}</span>
                            </div>
                        </div>
                        {tiersExpanded && (
                            <>
                        ...
                        <div onClick={() => setAutoListExpanded((v) => !v)} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", cursor: "pointer" }}>
                            ...
                            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                                {autoIncentiveProducts.length > 0 && (
                                    <>
                                        <button onClick={(e) => { e.stopPropagation(); printIncentiveList(); }} title="طباعة القائمة"
                                            style={{ background: "none", border: `1px solid ${COLORS.border}`, borderRadius: 6, color: COLORS.textDim, fontSize: 11, padding: "4px 8px", cursor: "pointer" }}>
                                            🖨️ طباعة
                                        </button>
                                        <button onClick={(e) => { e.stopPropagation(); shareIncentiveList(); }} title="مشاركة القائمة"
                                            style={{ background: "none", border: `1px solid ${COLORS.border}`, borderRadius: 6, color: COLORS.textDim, fontSize: 11, padding: "4px 8px", cursor: "pointer" }}>
                                            📤 مشاركة
                                        </button>
                                    </>
