...
export function CustomersModule({
    ...
    // 🆕 بقت بتحسب من props sales/creditPayments الجاهزين بدل query مباشر لـ Supabase —
    // نفس فلسفة تصحيح CreditTab بالظبط (تعليق أعلى الملف). ده كمان بيخليها تشتغل أوفلاين
    // تلقائيًا لأن sales/creditPayments أصلًا محمّلين ومتاحين للموديول من غير حاجة لنت وقت الفتح.
    const openCreditModal = (customer) => {
        setSelectedCreditCustomer(customer);

        const ajilSales = (sales || []).filter(
            (s) => s.customer === customer.id && s.payment === "آجل"
        );
    ...
    // 🆕 سداد آجل أوفلاين-أول — كتابة فورية في الكاش المحلي (credit_payments_cache + sales_cache)
    // + queueEvent بدل نداء مباشر لـ Supabase. بنستخدم event types موجودة بالفعل جوه offlineSync.ts
    // (CREDIT_PAYMENT_INSERT و SALE_INSERT) بدل ما نضيف نوع جديد، لأن الشكل نفسه مطابق تمامًا.
    const payCreditInvoice = async () => {
        ...
        try {
            await window.offlineAPI.upsertCreditPaymentCache(paymentRow);
        } catch (err) {
            console.error("upsertCreditPaymentCache failed:", err);
        }
        await queueEvent({
            id: paymentRow.id,
            type: "CREDIT_PAYMENT_INSERT",
            pharmacy_id: pharmacyId, // 🆕 لازم يكون على مستوى الـ event نفسه (نفس درس الشفت)
            timestamp: nowISO,
            payload: { records: [paymentRow] },
        });
        ...
        try {
            await window.offlineAPI.insertSaleCache(paymentRecord);
        } catch (err) {
            console.error("insertSaleCache (credit payment) failed:", err);
        }
        await queueEvent({
            ...
            timestamp: nowISO,
            payload: { invoice: paymentRecord },
        });
        setSales((p) => [...p, paymentRecord]);

        setCreditPayments((p) => [...p, paymentRow]);
        // تحديث الفواتير
        setCreditInvoices((p) =>
            p
                .map((inv) =>
        ...
        showToast(
            navigator.onLine
                ? "تم تسجيل السداد ✓"
                : "تم تسجيل السداد ✓ (هتتم المزامنة عند توفر الاتصال)"
        );
    };
...
هل أنت متأكد من الحذف؟`)) return;
    ...
    const save = async () => {
        ...
        const nowISO = new Date().toISOString();
        try {
            await window.offlineAPI.upsertCustomerCache(saved);
        } catch (err) {
            console.error("upsertCustomerCache failed:", err);
        }
        ...
        if (editing) {
            ...
            await queueEvent({
                ...
                pharmacy_id: pharmacyId, // 🆕 على مستوى الـ event نفسه
                timestamp: nowISO,
                payload: { id: editing, updates: saved, pharmacy_id: pharmacyId },
            });
            logAudit({
        ...
        showToast(
            navigator.onLine
                ? (editing ? "تم تعديل العميل ✓" : "تمت إضافة العميل ✓")
                : (editing ? "تم تعديل العميل ✓ (هتتم المزامنة عند توفر الاتصال)" : "تمت إضافة العميل ✓ (هتتم المزامنة عند توفر الاتصال)")
        );
    };
                            ...
                            {[
                                ...
                                <div
                                    ...
                                    style={{
                                        ...
                                        padding: "10px 8px",
                                        borderRadius: 10,
                                        border: `2px solid ${form.category === opt.val ? COLORS.blue : COLORS.border
                                            }`,
                                        background:
