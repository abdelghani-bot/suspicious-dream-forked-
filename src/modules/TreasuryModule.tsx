...
export function TreasuryModule({ sales, creditPayments, purchases, suppliers, pharmacyId, currentUser, users = [], showToast, shifts, entries, setEntries, returns = [], products = [], canViewSub = (_sub) => true, canEditSub = (_sub) => true, canAddSub = (_sub) => true, canDeleteSub = (_sub) => true }) {
    ...
    const [showAllOpeningHistory, setShowAllOpeningHistory] = useState(false);
    ...
    const [showAllSettlementHistory, setShowAllSettlementHistory] = useState(false);
        ...
        <div>
                ...
                <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                    {canAddOpeningBalance && (
                        <button
                            ...
                            style={{ position: "relative", width: 34, height: 34, borderRadius: "50%", background: COLORS.goldSoft, backdropFilter: "blur(10px)", WebkitBackdropFilter: "blur(10px)", border: `1px solid ${tint(COLORS.gold, 0.35)}`, color: COLORS.gold, cursor: "pointer", fontSize: 15, display: "flex", alignItems: "center", justifyContent: "center", padding: 0 }}>
                            ➕
                        </button>
                    )}
                    {canSettleBalance && (
                        <button
                            ...
                            title="تسوية الرصيد"
                            style={{ position: "relative", width: 34, height: 34, borderRadius: "50%", background: COLORS.blueSoft, backdropFilter: "blur(10px)", WebkitBackdropFilter: "blur(10px)", border: `1px solid ${tint(COLORS.blue, 0.35)}`, color: COLORS.blue, cursor: "pointer", fontSize: 15, display: "flex", alignItems: "center", justifyContent: "center", padding: 0 }}>
                            ⚖️
                            {settlementHistory.length > 0 && (
                                <span style={{ position: "absolute", top: -2, insetInlineEnd: -2, width: 9, height: 9, borderRadius: "50%", background: COLORS.red, border: `1.5px solid ${COLORS.surface}` }} />
                            )}
                        </button>
                    )}
                </div>
            ...
            {showSettlementForm && (
                <Modal open title="⚖️ تسوية رصيد الخزنة" onClose={() => !savingSettlement && setShowSettlementForm(false)}>
                    ...
                    {/* ── سجل تسويات رصيد الخزنة (جوه نفس المودال) ── */}
                    {settlementHistory.length > 0 && (
                        <div style={{ marginTop: 20, paddingTop: 16, borderTop: `1px solid ${COLORS.border}` }}>
                            <div style={{ fontWeight: 800, fontSize: 13, color: COLORS.textPrimary, marginBottom: 8 }}>📜 سجل التسويات السابقة</div>
                            {(showAllSettlementHistory ? settlementHistory : settlementHistory.slice(0, 15)).map((e) => (
                            ...
                            {settlementHistory.length > 15 && (
                                <button onClick={() => setShowAllSettlementHistory((v) => !v)} style={{ background: "transparent", border: "none", color: COLORS.blue, fontSize: 12, fontWeight: 700, cursor: "pointer", marginTop: 8, padding: 0 }}>
                                    {showAllSettlementHistory ? "عرض أقل" : `عرض الكل (${settlementHistory.length})`}
                                </button>
                            )}
                        </div>
                    )}
            ...
            {showOpeningBalanceForm && (
                <Modal open title="➕ إضافة رصيد أول المدة للخزنة" onClose={() => !savingOpeningBalance && setShowOpeningBalanceForm(false)}>
                    ...
                    {/* ── سجل رصيد أول المدة والإضافات (جوه نفس المودال) ── */}
                    {openingBalanceHistory.length > 0 && (
                        <div style={{ marginTop: 20, paddingTop: 16, borderTop: `1px solid ${COLORS.border}` }}>
                            <div style={{ fontWeight: 800, fontSize: 13, color: COLORS.textPrimary, marginBottom: 8 }}>📜 سجل الإضافات السابقة</div>
                            {(showAllOpeningHistory ? openingBalanceHistory : openingBalanceHistory.slice(0, 15)).map((e) => (
                            ...
                            {openingBalanceHistory.length > 15 && (
                                <button onClick={() => setShowAllOpeningHistory((v) => !v)} style={{ background: "transparent", border: "none", color: COLORS.blue, fontSize: 12, fontWeight: 700, cursor: "pointer", marginTop: 8, padding: 0 }}>
                                    {showAllOpeningHistory ? "عرض أقل" : `عرض الكل (${openingBalanceHistory.length})`}
                                </button>
                            )}
                        </div>
                    )}
